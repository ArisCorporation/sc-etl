...
## Ausführung
```bash
pnpm run dev -- --channel=LIVE --version=4.3.1
```
Optionale Flags:
...
4. **Load** (`src/load.ts`):
   - Stellt sicher, dass der Build in Directus (`game_builds`) existiert (`ensureBuild`).
...
Jede View liefert eine künstliche `pk`, sortiert je Channel nach `game_builds.released` (Fallback `ingested`) und zeigt nur den aktuellsten Stand pro Entität. Für `vw_installed_items_latest` muss – wie oben erwähnt – ein `item`-Feld auf `installed_items` vorhanden sein.
...
5. **Load beobachten** – Loader schreibt in die Directus-Collections (`companies`, `ships`, `ship_variants`, `items`, `hardpoints`, `item_stats`, `ship_stats`, `locales`, `game_builds`).
